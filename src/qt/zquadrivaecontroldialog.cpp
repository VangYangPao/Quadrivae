...
#include "ui_zquadrivaecontroldialog.h"
...
std::list<std::string> ZQuadrivaeControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZQuadrivaeControlDialog::listMints;

ZQuadrivaeControlDialog::ZQuadrivaeControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZQuadrivaeControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZQuadrivaeControlDialog::~ZQuadrivaeControlDialog()
{
    delete ui;
}

void ZQuadrivaeControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZQuadrivaeControlDialog::updateList()
{
...
void ZQuadrivaeControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZQuadrivaeControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZQuadrivae_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZQuadrivaeControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZQuadrivaeControlDialog::GetSelectedMints()
{
...
void ZQuadrivaeControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
