...
#include "zquadrivaecontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zQUAD ought to be enough for anybody." - Bill Gates, 2017
    ui->zQUADpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzQUADSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzQUAD->setEnabled(false);
        ui->pushButtonMintzQUAD->setToolTip(tr("zQUAD is currently disabled due to maintenance."));

        ui->pushButtonSpendzQUAD->setEnabled(false);
        ui->pushButtonSpendzQUAD->setToolTip(tr("zQUAD is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zQUADpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzQUAD_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zQUAD is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zQUAD...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zQUAD in ") +
...
void PrivacyDialog::on_pushButtonSpendzQUAD_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zQUAD is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zQUAD
        sendzQUAD();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zQUAD
    sendzQUAD();
}

void PrivacyDialog::on_pushButtonZQuadrivaeControl_clicked()
{
    ZQuadrivaeControlDialog* zQuadrivaeControl = new ZQuadrivaeControlDialog(this);
    zQuadrivaeControl->setModel(walletModel);
    zQuadrivaeControl->exec();
}

void PrivacyDialog::setZQuadrivaeControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzQuadrivaeSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzQUAD()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Quadrivae Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zQUADpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zQUAD is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " QUAD </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zQUAD</b>";
    ...
    // use mints from zQuadrivae selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZQuadrivaeControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZQuadrivaeControlDialog::GetSelectedMints();
    }
    ...
    // Clear zquadrivae selector in case it was used
    ZQuadrivaeControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zQuadrivae Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Quadrivae, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zQuadrivae Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zQUAD </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zQUAD "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zQUAD "));
    ui->labelzQUADAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzQUADSyncStatus->setVisible(fShow);
}
